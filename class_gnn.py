...
# ===============================================================
class DynamicGNNLine(nn.Module):
...
class HybridGnnTorsoLine(nn.Module):
    ...
    def __init__(self, S=4, T=8, dim_c=32,gnn_layers=1, n_tf_layers=2, n_heads=4):
        ...
        # dynamic GNN
        self.gnn = DynamicGNNLine(in_dim=5, out_dim=dim_c,
...
#  HybridGnnTorsoV2  (GNN + transformer on past actions + scalar)
# ===============================================================
class HybridGnnTorsoATT(nn.Module):
    """
      • GNN (GraphSAGE) on current frame → 48 embeddings
      • Transformer on past actions (T-1 frames) → 1 embedding
      • Linear on moves_left scalar → 1 embedding
...
class DynamicGNNATT(nn.Module):
    def __init__(self, in_dim=5, out_dim=32, num_layers=3, heads=4):
        super().__init__()
        self.heads = heads
        ...
        for i in range(num_layers):
            in_c = out_dim if i > 0 else out_dim
            self.convs.append(GATConv(in_c, out_dim // heads, heads=heads, concat=True))  # output dim = out_dim
        self.norm = nn.LayerNorm(out_dim)
